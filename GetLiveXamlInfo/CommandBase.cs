...
#pragma warning disable SA1501 // Statement should not be on a single line
                    ...
                    var dockView = typeof(Microsoft.VisualStudio.Platform.WindowManagement.DTE.WindowBase).GetProperty("DockViewElement", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                    if (dockView is null) { return new List<string>() { "Unexpected situation: Unable to access DockViewElement type." }; }

                    ////var proDve = dockView.GetValue(proWindow, null);
                    ...
                    var dve = dockView.GetValue(window, null);
                    ...
                    var toolViewWindow = dve as Microsoft.VisualStudio.Platform.WindowManagement.ToolWindowView;
                    if (toolViewWindow is null) { return new List<string>() { "Unexpected situation: Unable to access the ToolViewWindow for the Live XAML Tree Window." }; }

                    var dc = ((System.Windows.FrameworkElement)toolViewWindow.Content).DataContext;
                    ...
                    var xamlDiagHost = (Microsoft.VisualStudio.DesignTools.Diagnostics.Model.IXamlDiagnosticsHost)hostField.GetValue(treeVm);
                    if (xamlDiagHost is null) { return new List<string>() { "Unexpected situation: Unable to access Xaml Diagnostics Host." }; }

                    var poc = new Microsoft.VisualStudio.DesignTools.Diagnostics.UI.LivePropertyExplorer.Model.PlatformObjectCache(xamlDiagHost.ProtocolHandler, xamlDiagHost.TelemetryLogger);
                    if (poc is null) { return new List<string>() { "Unexpected situation: Unable to access the LivePropertyExplorer.Model.PlatformObjectCache." }; }

                    ////var propExplr = new Microsoft.VisualStudio.DesignTools.Diagnostics.UI.LivePropertyExplorer.ViewModel.PropertyExplorerViewModel(xamlDiagHost);
                    ////if (propExplr is null) { return new List<string>() { "Unexpected situation: Unable to access the LivePropertyExplorer.ViewModel.PropertyExplorerViewModel." }; }
#pragma warning restore SA1501 // Statement should not be on a single line
                    ...
                    async Task<List<string>> OutputElement(Microsoft.VisualStudio.DesignTools.Diagnostics.UI.LiveVisualTree.ViewModel.IElementViewModel element, int depth)
                    {
                        ...
                        {
                            var propObj = await poc.GetElementOrPropertyObjectAsync(element.Element.Handle);
                            ...
                            foreach (var source in propObj.Sources)
                            {
                                ...
                                if (sname != "Default")
                                {
                                    if (detailLevel == DetailLevel.All || sname == "Local")
                                    {
                                        ...
                                        {
                                            if (sourceProp.Name == "Content")
                                            {
                                                continue;
                                            }

                                            if (sourceProp.IsHandle)
                                            {
                                                var innerProp = await poc.GetElementOrPropertyObjectAsync(sourceProp.Handle);

                                                if (innerProp != null)
                                                {
                                                    var complexTypeName = string.IsNullOrWhiteSpace(sourceProp.ValueTypeName) ? sourceProp.TypeName : sourceProp.ValueTypeName;
                                                    properties.Add($"{new string(' ', depth)} {sourceProp.Name} = '{complexTypeName}'");

                                                    foreach (var ips in innerProp.Sources)
                                                    {
                                                        foreach (var ipsp in ips.Properties)
                                                        {
                                                            properties.Add($"{new string(' ', depth + 1)} [{ipsp.Name} = {ipsp.Value ?? "[null]"}]");
                                                        }
                                                    }
                                                }
                                            }
